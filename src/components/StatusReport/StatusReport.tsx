...
export const StatusReport = ({ lessonsStatus }: StatusReportProps): ReactElement => {
  if (!lessonsStatus) {
    return;
  }
  ...
  const correctPercentage = totalLines > 0 ? (correctLines / totalLines) * 100 : 0;
  const incorrectPercentage = totalLines > 0 ? (incorrectLines / totalLines) * 100 : 0;

  return (
    <div className={styles.StatusReport}>
      <div className={styles.ProgressContainer}>
        <div className={styles.ProgressBar}>
          <div className={styles.ProgressFillCorrect} style={{ width: `${correctPercentage}%` }} />
          <div
        ...
        <div className={styles.ProgressText}>
          {correctLines} / {totalLines} lines correct ({Math.round(correctPercentage)}%)
        </div>
      </div>
