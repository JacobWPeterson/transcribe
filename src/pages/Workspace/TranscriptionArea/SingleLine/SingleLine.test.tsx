...
describe('SingeLine', () => {
  it('renders correctly for titles', async () => {
    render(<SingleLine line={mockTitleLine} passedIndex={0} updateLessonStatus={() => {}} />);
  ...
  it('shows a help message when user types non-Greek characters', async () => {
    ...
    expect(screen.getByText(/Non-Greek characters have been detected/)).toBeInTheDocument();
    expect(screen.getByRole('link', { name: 'Help' })).toBeInTheDocument();

    await user.clear(lineInput);
    expect(screen.queryByText(/Non-Greek characters have been detected/)).not.toBeInTheDocument();
    expect(screen.getByText(/Helpful caption/)).toBeInTheDocument();
  });
  ...
  it('tells the user their incorrect answer is too short when appropriate', async () => {
    ...
    expect(screen.getByText('Your answer is 1 letter too short.')).toBeInTheDocument();
  });
  ...
  it('tells the user their incorrect answer is too long when appropriate', async () => {
    ...
    expect(incorrectButton).toBeInTheDocument();
    await user.hover(incorrectButton);
    expect(screen.getByText('Your answer is 2 letters too long.')).toBeInTheDocument();
  });
  ...
  it('shows a hint after three incorrect guesses of correct length', async () => {
    ...
    await user.hover(screen.getByRole('img', { name: 'help' }));
    expect(screen.getByText('Incorrect letters: ω(6), ζ(7).')).toBeInTheDocument();
  });
  ...
  describe('a11y', () => {
    it('has keyboard accessible answer checking', async () => {
      // Answer is αδελφοσ
      render(
        <SingleLine line={mockNewConceptLine} passedIndex={1} updateLessonStatus={() => {}} />
      );
    ...
    it('has keyboard accessible title messaging', async () => {
      // Answer is αδελφοσ
      render(<SingleLine line={mockTitleLine} passedIndex={1} updateLessonStatus={() => {}} />);
