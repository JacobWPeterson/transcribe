...
describe('Resources', () => {
  it('should render correctly', () => {
    ...
    expect(screen.getByRole('heading', { level: 1, name: 'Test Resources' })).toBeInTheDocument();

    // Websites (i.e., not a journal)
    expect(screen.getByRole('heading', { level: 2, name: 'Heading 1' })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: mockResources[0].resources[0].name })).toHaveAttribute(
      'href',
      mockResources[0].resources[0].url
    );
    expect(screen.getByText(`by ${mockResources[0].resources[0].creator}`)).toBeInTheDocument();
    expect(screen.getByText(mockResources[0].resources[0].description)).toBeInTheDocument();
